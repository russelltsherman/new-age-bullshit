...
func sellingPoint() []string {
	return []string{
		"Through nSubject, our nOurPlural are ppThingPrep nMass.",
		"nSubject may be the solution to what is holding you back from a adjBig nOf of nMass.",
...
// NewSentencePool slice of strings
func NewSentencePool() [][]string {
	var pool [][]string
	pool = append(pool, explaining())
	pool = append(pool, warning())
	pool = append(pool, futureHope())
	pool = append(pool, youAndYourProblem())
	pool = append(pool, history())
	pool = append(pool, sellingPoint())
	return pool
}
